...
class VideoReader:
    def __init__(self, camera_id):
        ...
        self.serial_sender2 = SerialSender()
        self.status = 0
    ...
    def execute(self):
        ...
        while True:
            ...
            if line:
                c_line = str(line.decode())
                print(f'line: {str(line.decode())}')
                EN_ID = str(line.decode()[0:8])
                EN_DATA = str(line.decode()[9:24])
                
                try:
                    EN_strength = int(line.decode()[25:28])
                    with open(self.output_path, mode="a") as f:
                        # row_data = f'{l_pow},{","},{r_pow}\n'
                        #f.write(row_data)
                        now = datetime.datetime.now()
                        f_line=f'{now.strftime("%Y/%m/%d")},{now.strftime("%H:%M:%S")},{EN_ID},{EN_DATA},{EN_strength}\n'
                        f.write(f_line)
                        print("id="+EN_ID )
                        print("data="+EN_DATA )
                        print(EN_strength )

                        dac_dt=EN_strength*-1+200

                        dac_set=str(dac_dt)+"/"+str(dac_dt)
                        back_data=str(EN_strength)+"/"+str(EN_strength)
                        self.serial_sender.send(back_data)
                        #self.serial_sender.send2(dac_set)
                        #print(back_data)
                except:
                    pass
            ...
            line2= self.serial_sender.read()
            if line2:
                print(f'set_pw: {str(line2.decode())}')
