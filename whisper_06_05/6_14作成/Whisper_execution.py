...
#カレントディレクトリにver_test.txtがあれば削除する
if os.path.exists("ver_test.txt") == True:
            os.remove("ver_test.txt")
else:
    pass
...
#ユーザーセッティング
Setings = sg.UserSettings(filename="data_file",path=os.path.split(__file__)[0])
Setings.load()

lay_2 = sg.Tab("Gladia_API",[
    [sg.Frame("",[
    [sg.Text("APIキー"),sg.InputText(default_text=Setings["API_key"],key="API_key",size=(25,1),),sg.Button("APIキー保存",key="API_save")],
    ...
    [sg.Frame("",layout=[
        [sg.Text("ファイル保存先")],
        [sg.InputText(key="api_dl_path",size=(30,1)),sg.FolderBrowse("選択"),sg.Button("ダウンロードする",key="api_dl_bt",button_color=("white","blue"))],
    ],key="api_dl",visible=False)],
...
while True:
    ...
    def GO (file_path):
        ...
        out_put = str(out.stdout,"utf-8",)#"shift_jis"
    ...
    if event == "START":
        ...
        with open("ver_test.txt","r",encoding="utf-8") as f:
            out = f.read()
...
#-+-+-+-+-+-+-+-+-+-+-+-Gladia_API実行処理-+-+-+-+-+-+-+-+-+-+-+-+-
    ...
    if event == "api_start":
        ...
        if value["API_in_file"] == "":
            ...
            if value["toggle_diarization_off"] == True:
                ...
                #ダウンロードボタンを表示する
                window["api_dl"].update(visible=True)
    ...
    if event == "api_dl_bt":
        if value["api_dl_path"] == "":
            ...
            output_file_name = os.path.join(value["api_dl_path"],f"{extension_file_name}.txt")
            #テキスト出力
            f = open(output_file_name,"w")
            f.write(value["api_out"])
            f.close()
            
    #APIキー保存を押した時の処理
    if event == "API_save":
        Setings["API_key"] = value["API_key"]
